...
import { createExclusiveAccess } from "./create-exclusive.js";

const CREATE_GATE_CONFIGURATION_MUTATION = `
  mutation createGateConfiguration($name: String!, $requirements: String!, $reaction: String!, $gateType: String!) {
    ...
    gateSubjectCreate(input: {
      ...
      gateSubject {
        ...
        configuration {
          ...
          gateType: metafield(namespace: "${myAppMetafieldNamespace}",
            key: "gateType") {
              value
          }
          createdAt
...
export default async function createGate({
  ...
  const gateDiscountConfigurationReaction = {
    name: name,
    type: "discount",
    discount: {
      type: discountType,
  ...
  const gateExclusiveConfigurationReaction = {
    name: name,
    type: "exclusive",
    purchase_limit: 1,
  };
  ...
  try {
    const gateConfigurationReaction =
      perk === "exclusive"
        ? gateExclusiveConfigurationReaction
        : gateDiscountConfigurationReaction;
